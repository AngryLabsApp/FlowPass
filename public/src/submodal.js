...
(function () {
      ...
      <form class="form" id="sub-update-form">
        <div class="form__row">
          ...
          <select class="form__control" id="sub-Plan" name="Plan">
            ...
            <option>12 Sesiones Mensuales</option>
            <option>16 Sesiones Mensuales</option>
            <option>20 Sesiones Mensuales</option>
            <option>Clase Libre</option>
            <option>Clase Gratis</option>
            <option>12 Sesiones Personalizadas</option>
            <option>16 Sesiones Personalizadas</option>
            <option>20 Sesiones Personalizadas</option>
          </select>
        </div>
  ...
  function attachPlanFormHandlers(scope) {
    ...
    if (planEl && montoEl) {
      planEl.addEventListener('change', () => {
        ...
        try {
          const planObj = (typeof PLANES !== 'undefined') ? PLANES.find(item => item.value === p) : null;
    ...
    try {
      ...
      if (user) {
        const setVal = (sel, val) => { const el = scope.querySelector(sel); if (el && val != null) el.value = String(val); };
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        const payload = {
          id: user.id,
          dias_de_gracia: { value: scope.querySelector('#sub-Dias_de_Gracia')?.value || 0 },
          monto: { value: scope.querySelector('#sub-Monto')?.value || 0 },
          medio_de_pago: { value: scope.querySelector('#sub-Medio_de_pago')?.value },
          estado_pago: { value: scope.querySelector('#sub-PaymentStatus')?.value },
        };
        ...
        try {
          const patch = {
            dias_de_gracia: scope.querySelector('#sub-Dias_de_Gracia')?.value || 0,
            monto: scope.querySelector('#sub-Monto')?.value || 0,
            medio_de_pago: scope.querySelector('#sub-Medio_de_pago')?.value || '',
  ...
  function buildSingleFieldFormHTML(fieldKey) {
    const meta = (typeof FIELD_VALUES !== 'undefined') ? FIELD_VALUES[fieldKey] : undefined;
  ...
  function attachSingleFieldHandlers(scope, fieldKey) {
    ...
    try {
      const user = window.getUserSelected ? window.getUserSelected() : null;
      if (user) {
        const setVal = (id, val) => { const el = scope.querySelector(`#${id}`); if (el && val != null) el.value = String(val); };
        switch (fieldKey) {
